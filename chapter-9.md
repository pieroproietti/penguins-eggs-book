...
[vendor] contains the following structure:
...
`livecd`
The components for the grub and isolinux theme of the live image are given in livecd
...
(*) In EducaAndOS in order to have administration rights for the user, we need to configure the user in a specific group, so it became necessary to use users.yml.
